...
<div class="apk-vector-header" style="padding-bottom: 25px; padding-top: 90px; background-color: #d97706;">
    <div class="apk-top-logo">
        <div style="width: 45px; height: 45px; border-radius: 12px; background: #fff; color: #d97706; display: flex; align-items: center; justify-content: center; box-shadow: 0 4px 10px rgba(0,0,0,0.1);">
        ...
        <div>
            ...
            <div style="font-size: 1.2rem; font-weight: 800; color: #fff;">Log Poin Siswa</div>
        </div>
    </div>
</div>
...
<div class="apk-main-board" style="margin-top: -20px; border-radius: 25px 25px 0 0; min-height: 70vh; padding: 20px;">
    ...
    <div style="display: flex; gap: 10px; margin-bottom: 20px; background: #f1f5f9; padding: 5px; border-radius: 12px;">
        <button id="btnTabLog" onclick="switchTab('log')" style="flex: 1; padding: 10px; border: none; border-radius: 8px; font-weight: 800; font-size: 0.9rem; background: #fff; color: #d97706; box-shadow: 0 2px 5px rgba(0,0,0,0.05); transition: 0.2s;">Riwayat Log</button>
        <button id="btnTabTop" onclick="switchTab('top')" style="flex: 1; padding: 10px; border: none; border-radius: 8px; font-weight: 800; font-size: 0.9rem; background: transparent; color: #64748b; transition: 0.2s;">Top Siswa</button>
    </div>
    ...
    <div id="tabLog">
        <?php if (empty($pelanggaran)): ?>
            <div style="text-align: center; padding: 40px 20px; color: #64748b;">
                ...
                <p style="margin: 0; font-size: 0.9rem;">Belum ada pelanggaran yang tercatat.</p>
            </div>
        <?php else: ?>
    ...
    <div id="tabTop" style="display: none;">
        <?php if (empty($topSiswa)): ?>
            <div style="text-align: center; padding: 40px 20px; color: #64748b;">
                ...
                <h4 style="margin: 0 0 10px 0;">Semua Siswa Disiplin</h4>
                <p style="margin: 0; font-size: 0.9rem;">Belum ada akumulasi poin tercatat.</p>
            </div>
        <?php else: ?>
            <div style="display: flex; flex-direction: column; gap: 12px;">
                <?php $rank = 1; foreach ($topSiswa as $ts): ?>
                    ...
                    <div style="background: <?= $bgColor ?>; padding: 15px; border-radius: 16px; border: 1px solid <?= $borderColor ?>; display: flex; align-items: center; gap: 15px;">
                        ...
                        <div style="flex: 1;">
                            ...
                            <div style="font-size: 0.8rem; font-weight: 600; color: #64748b; display: flex; align-items: center; gap: 5px;">
                                <i data-lucide="users" style="width: 12px; height: 12px;"></i> Kelas <?= htmlspecialchars($ts['nama_kelas']) ?>
                            </div>
                        </div>
...
function switchTab(tab) {
    ...
    document.getElementById('btnTabLog').style.color = tab === 'log' ? '#d97706' : '#64748b';
    ...
    document.getElementById('btnTabTop').style.color = tab === 'top' ? '#d97706' : '#64748b';
